...
const add = async (req, res) => {
    ...
    try {
        ...
        const sqlInsert = "INSERT INTO meetdoctor (title, title_ar, description,description_ar,image) VALUES (?, ?, ?, ?, ?)";
...
const get = async (req, res) => {
    const sqlSelect = 'SELECT * FROM meetdoctor';
...
const deleteMeetDoctor=async(req,res)=>{
    const id=req.params.id
    const sqlDelete="DELETE FROM meetdoctor WHERE id = ? ";
...
const update = async (req, res) => {
    ...
    const checkSql = 'SELECT * FROM meetdoctor WHERE id = ?';
    db.query(checkSql, [id], async(checkErr, checkResult) => {
        if (checkErr) {
            ...
            if (checkResult.length === 0) {
                ...
                const sqlUpdate = 'UPDATE meetdoctor SET title = ?, title_ar = ?, description = ?,description_ar = ?, image = ? WHERE id = ?';
...
module.exports={add,get,update,deleteMeetDoctor}